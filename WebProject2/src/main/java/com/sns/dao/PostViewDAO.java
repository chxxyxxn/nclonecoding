...
public class PostViewDAO {
	...
	public ArrayList<PostViewDTO> timeLine(int loginID) throws Exception {
		...
		String sql = "select p.nickname, p.profile_id, p.profile_img, ps.content, ps.img_file_name, ps.post_id ,like_cnt,to_char(ps.write_date, 'mm.dd HH24:mi') as writedate "
				...
				+ " and ps.writer = f.following " + " and f.follower = ?" + " union"
		...
		while (rs.next()) {
			...
			PostViewDTO pDto = new PostViewDTO(postID, writer, writeDate, content, profileImg, imgFile, likeCnt,
					profileID);
			postList.add(pDto);
	...
	public ArrayList<ProfileDTO> viewProfile(int loginId) throws Exception {
		...
		Connection conn = DBConnection.getConnection();
		String sql = "select profile_img, nickname, back_img, statemsg from profile where profile_id= ?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, loginId);
		ResultSet rs = pstmt.executeQuery();
	...
	public ArrayList<StartProfileDTO> startProfile(int memberId) throws Exception {
		...
		String sql = "select profile_id, member_id, profile_img, nickname, back_img, statemsg from profile where member_id= ?";
		...
		while (rs.next()) {
			...
			StartProfileDTO prDto = new StartProfileDTO(profileID, nickname, profileImg, backImg, stateMsg, memberID);
	...
	//멤버아이디<>프로필아이디
	public int alterProfile(int profileId) throws Exception {
		int memberID=0;
		...
		String sql = "select member_id from profile where profile_id =?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	// 내가 쓴 글
	public ArrayList<PostViewDTO> myPost(int userID) throws Exception {
		...
		while (rs.next()) {
			...
			int likeCnt = rs.getInt("like_cnt");
			int profileID = rs.getInt("profile_id");
			PostViewDTO mpDto = new PostViewDTO(postID, writer, writeDate, content, profileImg, imgFile, likeCnt,
					profileID);
	...
	// 좋아요 누른 게시글
	public ArrayList<PostViewDTO> likePost(int userID) throws Exception {
		...
		String sql = "select content, nickname, p.profile_id,to_char(write_date,'mm.dd HH24:MI') as writedate, profile_img, img_file_name, post_id , like_cnt"
				...
				+ " and ps.post_id = lmp.like_post_id " + " and lmp.like_user = ? order by writedate desc";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, userID);
		ResultSet rs = pstmt.executeQuery();
		...
		while (rs.next()) {
			...
			int likeCnt = rs.getInt("like_cnt");
			int profileID = rs.getInt("profile_id");
			PostViewDTO lpDto = new PostViewDTO(postID, writer, writeDate, content, profileImg, imgFile, likeCnt,
					profileID);
	...
	public ArrayList<HashtagPostDTO> searchPost(String keyword) throws Exception {
		...
		String sql = "select  p.nickname, ps.like_cnt, p.profile_img, ps.content, ps.img_file_name, ps.post_id ,to_char(ps.write_date, 'mm.dd HH24:mi') as writedate, ht.hashtag_name "
				...
				+ " and p.profile_id = ps.writer " + "	and ht.hashtag_name like ? ";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		...
		while (rs.next()) {
			...
			HashtagPostDTO spDto = new HashtagPostDTO(postID, writer, writeDate, content, profileImg, imgFile, likeCnt,
					hashtag);
	...
	// 팔로잉 수
	public int getFollowingCnt(int loginID) throws ClassNotFoundException, SQLException {
	...
	// 팔로워 수
	public int getFollowerCnt(int loginID) throws Exception {
		Connection conn = DBConnection.getConnection();
	...
	public ArrayList<PostViewDTO> getTrendPost() throws Exception {
		...
		String sql = "select distinct  ps.post_id,p.profile_id, to_char(write_date,'mm.dd') as writedate, like_cnt, img_file_name, content , p.nickname, profile_img "
				+ " from post ps, profile p, post_hashtag ht" + " where p.profile_id = ps.writer"
				+ " and ht.trend_date = ps.write_date" + " and ps.post_id = ht.post_id"
		...
		while (rs.next()) {
			...
			int likeCnt = rs.getInt("like_cnt");
			int profileID = rs.getInt("profile_id");
			PostViewDTO trendDto = new PostViewDTO(postID, writer, writeDate, content, profileImg, imgFile, likeCnt,
					profileID);
	...
	public ArrayList<PostViewDTO> thisPost(int PostID) throws Exception {
		...
		if (rs.next()) {
			...
			int likeCnt = rs.getInt("like_cnt");
			int profileID = rs.getInt("profile_id");
			PostViewDTO thisDto = new PostViewDTO(postID, writer, writeDate, content, profileImg, imgFile, likeCnt,
					profileID);
